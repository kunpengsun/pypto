...
`AssignTypeSymmetry` 验证器；这些诊断保留原有规则名称。先定义后使用检查采用严格
的词法作用域：分支内定义不能通过无区域结果的旧式形式逃逸。
对于 `WindowBuffer` 表达式，严格检查器在每个使用位置遍历 size，防止共享窗口隐藏
未定义或超出作用域的尺寸。分配的指针载体继续豁免；独立的旧式 `UseAfterDef` 行为不变。
表示和注册调用契约
